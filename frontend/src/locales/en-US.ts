export default {
  ...
  admin: {
    ...
    siteUsers: {
      title: 'Site Users',
      subtitle: 'Browse admin-site users and tag top-ups as gift / rebate / paid for revenue-side balance.',
      refresh: 'Refresh',
      needAdmin: 'Sign in to the dashboard admin account first',
      needAdminHelp: 'Site users use the same admin session as the dashboard for this workspace.',
      searchPlaceholder: 'Search email, user ID, or username…',
      search: 'Search',
      hideExcludedOn: 'Excluded users hidden',
      hideExcludedOff: 'Show excluded users',
      listTitle: 'Users',
      total: '{count} users',
      empty: 'No users, or no search matches.',
      selectHint: 'Select a user on the left, or pick a search candidate, to tag top-up history.',
      balance: 'Balance',
      notes: 'Note',
      lastUsed: 'Last used',
      ...
      nonRevenueHint: 'Revenue side deducts {value} (gift + rebate marks)',
      listGiftRebate: 'Gift/rebate ',
      listRechargeTotal: 'Lifetime recharge ',
      topupHelp: 'Credits: affiliate → rebate, clear gifts → gift, else → recharge. Debits are not auto-classified (could be refund or gift clawback)—tag as recharge refund / gift clawback / rebate clawback. Same tag uses signed amounts; totals are net. Concurrency/RPM excluded.',
      platformLifetime: 'Platform lifetime top-up',
      detailSum: 'Detail total',
      giftRebate: 'Gift + rebate',
      topupEmpty: 'No top-up history',
      suggested: 'Suggested: {tag}',
      sort: {
        id: 'User ID',
        balanceCny: 'Payout balance',
        recharge: 'Lifetime recharge',
        gift: 'Gift',
        rebate: 'Rebate',
      ...
      usage: {
        title: 'Usage detail',
        detailBtn: 'Detail',
        help: 'Actual spend by group for today and all history (platform units). Percent is share of the period total.',
        today: 'Today',
        history: 'History',
        total: 'Total',
      ...
      tags: {
        ...
        giftClawback: 'Gift clawback',
        rebateClawback: 'Rebate clawback',
      },
      errors: {
        network: 'Network error',
        request: 'Request failed, try again',
        platformUnsupported: 'This admin platform does not support site-user top-up history (Sub2API required).',
        historyUnavailable: 'Could not load top-up history. Check the admin session.',
        usageUnavailable: 'Could not load usage. Check the admin session.',
        invalidUser: 'Invalid user',
        invalidRequest: 'Invalid request',
        invalidTag: 'Invalid tag',
        invalidAmount: 'Invalid amount',
        invalidRate: 'Enter a valid recharge rate (greater than 0)',
      },
    },
    siteBalance: {
      title: 'Site user balances',
      subtitle: 'Sort users by balance; cost units = platform balance × site recharge rate.',
      rechargeRate: 'Rate',
      rechargeRateHelp: '1 platform unit = right-hand cost/CNY. Saved value feeds the dashboard site balance.',
      excludeAdmin: 'Exclude admins',
      excludeUsers: 'Exclude users',
      excludeUsersHelp: 'Test accounts etc. are excluded from site-user balance totals and hidden here. Search by email/ID to add.',
      excludeUsersPlaceholder: 'Email, user ID, or username…',
      saveSettings: 'Save',
      pageCost: 'Page payout total',
      pageRecharge: 'Page lifetime recharge',
      rateLabel: 'Current rate',
      colUser: 'User',
      colBalance: 'Platform',
      colBalanceCny: 'Payout balance',
      colRechargeTotal: 'Lifetime recharge',
      colGift: 'Lifetime gift',
      colRebate: 'Lifetime rebate',
      colAction: 'Action',
      markLedger: 'Tag top-ups',
    },
    leaderboard: {
    ...
    dashboard: {
      ...
      capital: {
        ...
        giftTotal: 'Lifetime gifts',
        rebateTotal: 'Lifetime rebates',
        rechargeTotal: 'Lifetime recharges',
        upstreamBalance: 'Upstream Balance',
        coverage: 'Balance Coverage',
        coverageHint: 'Prepaid reserve ÷ payout balance (current user balances incl. gift remaining; credit-line book balance excluded). N/A when user balance is zero.',
        creditReferenceHint: 'Credit-line platform book total (reference only): {value}',
      ...
      groupProfit: {
        ...
        subtitle: '{ownCount} own groups · Net profit {profit} · Margin {margin}',
        hint: 'Total revenue {revenue}. Own: admin group revenue − upstream key cost. Upstream: cost = key spend; revenue prefers real-connect admin account/channel today usage; else cost-share (actual margin equals parent). Margin primary = actual; secondary “Budget” = sale/cost multiplier theory.',
        revenueSourceAccount: 'Account actual',
        revenueSourceAllocated: 'Cost-allocated',
        budgetMargin: 'Budget {margin}',
        actualMarginLabel: 'Actual {margin}',
        actualMarginHint: 'Actual',
        allocatedMarginHint: 'Under cost-share, actual margin equals parent; primary figure is budget margin (sale/cost rates)',
        probeCost: 'incl. probe {cost}',
        expand: 'Expand upstreams',
        collapse: 'Collapse upstreams',
        upstreamCount: '{count} upstream',
        unmappedTitle: 'Unmapped upstreams',
        unmappedHint: 'Upstream groups with spend today but no own-group mapping. Cost only; no admin revenue.',
        upstreamPartial: 'Some upstream sites failed to report key usage; the list may be incomplete.',
      ...
      todayInbound: {
        ...
        empty: 'No inbound today: mark recharges on prepaid sites, or record settlements on credit-line sites.',
        loadError: 'Failed to load inbound breakdown.',
        colSite: 'Upstream site',
        colPlatform: 'Platform',
        colEntries: 'Entries',
        colAmount: 'Cost amount',
        total: 'Total',
      },
      upstreamKeyUsage: {
      ...
      balanceFilter: {
        ...
        subtitle: 'Cost side includes gifts/rebates; revenue side can deduct gift amounts. Whole-user exclude removes both.',
        close: 'Close',
        ...
        excludeUserIds: 'Exclude user IDs entirely',
        excludeUserIdsHelp: 'Test accounts etc.: excluded from both cost and revenue totals.',
        userIdPlaceholder: 'User ID',
        userGifts: 'Gift amounts (manual override)',
        userGiftsHelp: 'Revenue side only. Prefer tagging top-ups on Site Users (auto gift/rebate); this is an extra manual floor. Max(manual, marked) is used.',
        giftAmountPlaceholder: 'Gift amount',
        siteUsersLink: 'Tag top-ups on Site Users',
        add: 'Add',
        cancel: 'Cancel',
        save: 'Save',
        saving: 'Saving...',
    ...
    connectionHealth: {
      ...
      groupDetail: {
        ...
        probeAutomation: 'Run strategy probe now',
        probeAutomationRunning: 'Probing…',
        probeAutomationHint: 'Immediately run a full strategy probe for monitored accounts/channels in this group (writes state/events and may trigger auto-degrade or remote actions)',
        probeAutomationResult: 'Group probe finished: {probed} probed, {skipped} skipped, {failed} failed ({total} accounts/channels total)',
        ...
        metrics: {
          ...
          todaySpend: "Today's spend",
          todayProbeCost: "Today's probe cost",
          todayProbeCostHint: 'Total real probe spend for this group today (from upstream rates / response actual_cost, not an estimate rate)'
        },
        statusBreakdown: {
        ...
        columns: {
          ...
          upstreamSpendHint: "Today's actual cost for the upstream API key group linked to this account (summed across keys). My-group cost is the sum of upstream key spend mapped to that group.",
          upstreamSpendFor: 'Today\'s cost for upstream group "{group}"',
          upstreamProfit: 'Upstream group profit',
          upstreamProfitHint: "Profit for the linked upstream group (same as net-profit detail: prefers real-connect admin account usage − upstream key cost).",
          upstreamProfitFor: 'Today\'s profit for upstream group "{group}"',
          probeCost: 'Probe cost',
          probeCostHint: "Today's real probe spend for this account (event ledger: prefer upstream actual_cost, else tokens × group rate × recharge rate)",
          actions: 'Actions'
        },
      ...
      policies: {
        ...
        budgetCostUsage: 'Cost {used}/{total}',
        budgetCostUnlimited: 'unlimited',
        strategyModes: {
      ...
      policyDrawer: {
        ...
        dailyBudgetCostLabel: 'Daily cost budget (CNY)',
        dailyBudgetCostUsage: 'Spent today {used} / budget {total}, remaining {remaining}',
        dailyBudgetCostUnlimited: 'unlimited',
        ...
        priorityModes: {
          ...
          multiplier: 'Sort by Upstream Cost Multiplier'
        },
        priorityModeHelp: 'Multiplier sorting favors lower-cost upstream targets while they are healthy (preferring the linked upstream API key group rate). Failed targets always degrade before price ordering applies.',
        ...
        tooltips: {
          ...
          dailyBudget: 'Unit is real probe requests: how many live model probes this policy may run per China calendar day (UTC+8 reset). Each model probe costs 1 request. Once exhausted, real probes are skipped.',
          dailyBudgetCost: 'Unit is CNY. Max real probe spend per day. Cost is recorded from upstream response actual_cost, or tokens × upstream group rate × site recharge rate (USD units can differ by upstream, so the budget is always CNY). 0 means no money cap, but today\'s spent amount is still tracked and shown. When exhausted, real probes are skipped the same way as the request budget.',
      ...
      fields: {
        ...
        platform: 'Platform',
        balance: 'Balance',
        platformBalance: 'Platform book',
        platformBalanceHint: 'Reference only',
        todayConsume: 'Today Consume',
        historyRecharge: 'Lifetime Credits',
        groupName: 'Group Name',
        ...
        settlementModeCol: 'Mode',
      },
      sort: {
        label: 'Sort',
        balance: 'Balance',
        todayConsume: 'Today',
        historyRecharge: 'Lifetime',
      },
      status: {
      ...
      siteSettings: {
        ...
        balanceThresholdHelp: 'Enable to use a site-specific threshold. Disable to use the global default. Use a negative value (e.g. -1) to disable balance alerts for this site.',
        balanceThresholdPlaceholder: 'Amount, or -1 to never alert',
        settlementMode: 'Settlement / reserve mode',
        settlementModeHelp: 'Prepaid balances count toward coverage; credit-line book balance is reference-only with manual settlement.',
        modePrepaid: 'Prepaid wallet',
        modePrepaidHelp: 'Real top-ups; balance is usable reserve',
        modeCredit: 'Credit line',
        modeCreditHelp: 'Use first, settle later; platform book ≠ amount paid',
        creditLimit: 'Credit limit (cost units)',
        creditLimitPlaceholder: 'e.g. 10000',
        settlementCurrency: 'Settlement currency label',
        creditCostHint: 'Credit limit is in cost units after recharge rate, not raw upstream USD.',
        creditLimitNonNegative: 'Credit limit cannot be negative',
        save: 'Save',
        saveSuccess: 'Saved',
        saving: 'Saving...',
        cancel: 'Cancel'
      ...
      settlement: {
        ...
        open: 'Record settlement',
        costUnit: 'Cost units',
        outstanding: 'Outstanding',
        settled: 'Settled',
        consumed: 'Consumed',
        creditRemaining: 'Credit left',
        platformBalanceHint: 'Platform book (reference): {value}',
        register: 'Record a settlement',
        amountLabel: 'Amount',
        amountPlaceholder: 'Amount',
        settledAtLabel: 'Settlement time',
        noteLabel: 'Note',
        notePlaceholder: 'Note (WeChat / period, etc.)',
        submit: 'Confirm',
        manualHint: 'Book after offline payment; no auto charge. Settlement time decides which day counts as Today’s Inbound.',
        history: 'History',
        empty: 'No settlements yet',
        void: 'Void',
        voidConfirm: 'Void this settlement record?',
        delete: 'Delete',
        deleteConfirm: 'Permanently delete this settlement record? This cannot be undone.',
        invalidAmount: 'Enter a valid amount',
        invalidSettledAt: 'Pick a valid settlement time',
        loadingSummary: 'Loading settlement summary…',
      },
      ledger: {
        title: 'Inbound ledger',
        open: 'Inbound',
        costUnit: 'Cost units',
        todayInbound: 'Marked recharges (cost)',
        pageInbound: 'Marked recharges on this page',
        totalRecords: 'Detail rows',
        unmarked: 'Unmarked',
        unmarkedPage: '{count} unmarked on page',
        compareTitle: 'Reconciliation (cost units)',
        compareHint: 'Platform lifetime = History Recharge card (includes gifts/rebates). Only marked recharges count as inbound. Gaps usually mean incomplete details or gifts/rebates.',
        platformLifetime: 'Platform lifetime credits',
        detailListSum: 'Detail list total',
        markedRechargeSum: 'Marked recharges',
        markedOtherSum: 'Marked gift + rebate',
        gapLifetimeVsMarked: 'Lifetime − marked recharges = {value}',
        help: 'Auto-selects APIs by platform and loads full history: sub2api payment+redeem; new-api topup/self. Affiliate → rebate; others → recharge. Fix gifts manually. Only recharges count as inbound. UI is paginated.',
        platformRecords: 'Platform credit entries',
        empty: 'No credit entries',
        historyUnavailable: 'Could not load ledger history. Sync the site, or check new-api /api/user/topup/self or sub2api payment/redeem APIs.',
        needSession: 'Site is not connected. Sync the session first.',
        suggested: 'Suggested: {tag}',
        tags: {
          recharge: 'Recharge',
          gift: 'Gift',
      ...
      subscriptions: {
        ...
        remaining: 'left',
        todayMax: 'Max usable today',
        todayMaxHint: 'Min of remaining daily / weekly / monthly quotas',
        expires: 'Expires',
        starts: 'Start / renew',
        listSummary: '{count} subscription(s)',
        listSummaryMax: '{count} · max today {max} USD',
        topup: 'Record inbound',
        topupTitle: 'Subscription inbound · {name}',
        topupHint: 'Enter the actual paid amount (cost units). Use the start or renewal date as the inbound date so it counts on that day, not when you click submit.',
        topupAmount: 'Paid amount (cost)',
        topupAmountPlaceholder: 'e.g. 299',
        topupDate: 'Inbound date (start / renew)',
        topupDateHelp: 'Defaults to subscription startsAt. After renewal, a new startsAt creates a new date. You can override manually.',
        topupNotePlaceholder: 'Note (channel / order id)',
        topupSubmit: 'Confirm inbound',
        topupInvalidAmount: 'Enter a valid paid amount',
        topupInvalidDate: 'Enter a valid inbound date',
      },
      viewMode: {
        list: 'List Mode',
        card: 'Card Mode'
