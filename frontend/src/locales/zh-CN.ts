    ...
    connectionHealth: {
      ...
      groupDetail: {
        ...
        columns: {
          ...
          upstreamProfitHint: '该账号关联的上游分组今日估算利润（与仪表盘净利润下钻口径一致）',
      ...
      policyDrawer: {
        ...
        tooltips: {
          ...
          dailyBudgetCost: '单位是 USD（成本口径）：本策略每天最多允许消耗多少估算探活费用。填 0 表示不按金额限流，但仍会统计并展示今日已消耗金额。金额预算耗尽后同样会跳过真实探活。',
          probeCostPer1k: '用于估算探活费用：优先按上游返回的 usage tokens 计算 cost = tokens/1000 × 本费率；若响应无 usage，则按 max_tokens+16 估算。默认 0.002。',
